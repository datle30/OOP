...
public class Circle extends Shape {
     ...
     * Circle(chuyen dong).
     */
    public Circle(int x, int y, int radius, Color colorshape, int velocityX, int velocityY) {
        super(x, y, colorshape, velocityX, velocityY);
        this.radius = radius;
    }
    ...
    public void draw(Graphics g) {
        ...
        int xRound = (int) Math.round(x - radius);
        int yRound = (int) Math.round(y - radius);
        int radRound = (int) Math.round(radius * 2);
        g.fillOval(xRound, yRound, radRound, radRound);
    }
}
